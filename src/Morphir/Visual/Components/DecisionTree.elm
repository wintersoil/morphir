...
type Node
    = Branch BranchNode -- connectors
    | Leaf TypedValue --


type alias BranchNode =
    { condition : TypedValue --piece of IR
    , conditionValue : Maybe Bool --result of eval the condition (no args or args  =>  Maybe)
    , thenBranch : Node -- recursive
    , elseBranch : Node -- recursive
    }
...
numberOfNodes : Node -> Int
numberOfNodes node =
    case node of
        Branch branchNode ->
            numberOfNodes branchNode.thenBranch + numberOfNodes branchNode.elseBranch
...
{-|
    ...
    view value function to visualize each value in node
    root node is root of tree
    returns HTML
...
layoutHelp color viewValue rootNode =
    ...
    case rootNode of
        Branch branch ->
            ...
            -- 1) Order the then and else branches (copy and replicate the horizontalLayout
            -- 2) Number of nodes in the tree , the one with more nodes as the first one
            -- 3)
            if numberOfNodes branch.thenBranch > numberOfNodes branch.elseBranch then
                verticalLayout
                    (el
                        [ Border.width 2
                        , Border.rounded 7
