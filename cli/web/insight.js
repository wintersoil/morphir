...
var $author$project$Morphir$Web$Insight$init = function (distributionJson) {
	var model = function () {
		var _v0 = A2($elm$json$Json$Decode$decodeValue, $author$project$Morphir$IR$Distribution$Codec$decodeDistribution, distributionJson);
		if (_v0.$ === 'Ok') {
			var distribution = _v0.a;
			return $author$project$Morphir$Web$Insight$IRLoaded(distribution);
		} else {
			var error = _v0.a;
			return $author$project$Morphir$Web$Insight$Failed('Wrong IR');
		}
	}();
	...
	function (msg, model) {
		var getDistribution = function () {
			switch (model.$) {
				...
				case 'FunctionsSet':
					var visualizationState = model.a;
			...
			case 'FunctionNameReceived':
				...
				if (_v1.$ === 'Just') {
					...
					return A2(
						$elm$core$Maybe$withDefault,
						_Utils_Tuple2(
							$author$project$Morphir$Web$Insight$Failed('Invalid State in receiving function name'),
							...
							A2(
								...
								function (distribution) {
									return A2(
										$elm$core$Maybe$map,
										function (funDef) {
											return $author$project$Morphir$Web$Insight$FunctionsSet(
												{distribution: distribution, expandedFunctions: $elm$core$Dict$empty, functionArguments: _List_Nil, functionDefinition: funDef, selectedFunction: qName});
										},
					...
					return _Utils_Tuple2(
						$author$project$Morphir$Web$Insight$Failed('Received function name is not found'),
						$elm$core$Platform$Cmd$none);
				}
			case 'FunctionArgumentsReceived':
				var jsonList = msg.a;
				var jsonDecoder = $elm$json$Json$Decode$list(
					A2(
						$author$project$Morphir$IR$Value$Codec$decodeValue,
						$elm$json$Json$Decode$succeed(_Utils_Tuple0),
						$elm$json$Json$Decode$succeed(_Utils_Tuple0)));
				var _v2 = A2($elm$json$Json$Decode$decodeValue, jsonDecoder, jsonList);
				if (_v2.$ === 'Ok') {
					var updatedArgValues = _v2.a;
					if (model.$ === 'FunctionsSet') {
						var visualizationState = model.a;
						return _Utils_Tuple2(
							$author$project$Morphir$Web$Insight$FunctionsSet(
								_Utils_update(
									visualizationState,
						...
						return _Utils_Tuple2(
							$author$project$Morphir$Web$Insight$Failed('Invalid State'),
							$elm$core$Platform$Cmd$none);
					}
				} else {
					return _Utils_Tuple2(
						$author$project$Morphir$Web$Insight$Failed('Received function arguments cannot be decoded'),
						$elm$core$Platform$Cmd$none);
				}
			default:
				var fqName = msg.a;
...
var $author$project$Morphir$Visual$Context$fromDistributionAndVariables = F3(
	function (distribution, variables, onReferenceClick) {
		var references = $author$project$Morphir$Value$Interpreter$referencesForDistribution(distribution);
		return A4($author$project$Morphir$Visual$Context$Context, distribution, references, variables, onReferenceClick);
	});
...
var $author$project$Morphir$Visual$Components$DecisionTree$numberOfNodes = function (node) {
	if (node.$ === 'Branch') {
		var branchNode = node.a;
		return $author$project$Morphir$Visual$Components$DecisionTree$numberOfNodes(branchNode.thenBranch) + $author$project$Morphir$Visual$Components$DecisionTree$numberOfNodes(branchNode.elseBranch);
	} else {
		return 1;
	}
};
...
var $author$project$Morphir$Visual$Components$DecisionTree$verticalLayout = F7(
...
var $author$project$Morphir$Visual$Components$DecisionTree$layoutHelp = F3(
	function (color, viewValue, rootNode) {
		...
		if (rootNode.$ === 'Branch') {
			...
			return (_Utils_cmp(
				$author$project$Morphir$Visual$Components$DecisionTree$numberOfNodes(branch.thenBranch),
				$author$project$Morphir$Visual$Components$DecisionTree$numberOfNodes(branch.elseBranch)) > 0) ? A7(
				...
				elseColor,
				A3($author$project$Morphir$Visual$Components$DecisionTree$layoutHelp, elseColor, viewValue, branch.elseBranch)) : A7(
				$author$project$Morphir$Visual$Components$DecisionTree$verticalLayout,
				A2(
					$mdgriffith$elm_ui$Element$el,
					_List_fromArray(
...
var $author$project$Morphir$Visual$ViewReference$view = F3(
	function (context, viewValue, _v0) {
	...
	function (tag, value) {
		switch (value.$) {
			...
			case 'Apply':
				var fun = value.b;
				var arg = value.c;
				var _v2 = A2($author$project$Morphir$IR$Value$uncurryApply, fun, arg);
				var bottomFun = _v2.a;
				var args = _v2.b;
				var argsChild = A2(
					$elm$core$List$indexedMap,
					F2(
						function (index, argChild) {
							return A2(
								$author$project$Morphir$Visual$XRayView$valueToNode,
								$elm$core$Maybe$Just(
									A2(
				...
				return A3(
					...
					tag,
					$author$project$Morphir$Visual$XRayView$ValueNode(value),
					A2($elm$core$List$cons, funChild, argsChild));
			case 'Lambda':
				var arg = value.b;
				var body = value.c;
				return A3(
			...
			case 'LetDefinition':
				var flattenLet = function (v) {
					if (v.$ === 'LetDefinition') {
						...
						var def = v.c;
						var inValue = v.d;
						var _v4 = flattenLet(inValue);
						var subInValue = _v4.a;
						var subDefs = _v4.b;
						return _Utils_Tuple2(
							subInValue,
							A2(
				...
				var _v5 = flattenLet(value);
				var bottomInValue = _v5.a;
				var defs = _v5.b;
				return A3(
					$author$project$Morphir$Visual$XRayView$TreeNode,
					tag,
...
var $author$project$Morphir$Visual$XRayView$viewTreeNode = function (_v0) {
	var maybeTag = _v0.a;
	var nodeType = _v0.b;
	var treeNodes = _v0.c;
	var viewHeaderAndChildren = F2(
	...
	if (nodeType.$ === 'ValueNode') {
		...
		return A2(
			...
			A2($elm$core$List$map, $author$project$Morphir$Visual$XRayView$viewTreeNode, treeNodes));
	} else {
...
var $author$project$Morphir$Visual$XRayView$viewValue = function (value) {
	return A2(
		$mdgriffith$elm_ui$Element$el,
		_List_fromArray(
			[
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					$author$project$Morphir$Visual$XRayView$viewValue(other))
				]));
	});
...
var $author$project$Morphir$Visual$ViewValue$viewDefinition = F5(
	function (distribution, valueDef, variables, onReferenceClicked, expandedFunctions) {
		var ctx = A3($author$project$Morphir$Visual$Context$fromDistributionAndVariables, distribution, variables, onReferenceClicked);
		return A2(
			$mdgriffith$elm_ui$Element$column,
			_List_fromArray(
				[
			...
			_List_fromArray(
				[
					A3($author$project$Morphir$Visual$ViewValue$viewValue, ctx, variables, valueDef.body),
					$elm$core$Dict$isEmpty(expandedFunctions) ? $mdgriffith$elm_ui$Element$none : A2(
					$mdgriffith$elm_ui$Element$column,
					_List_fromArray(
						[
							$mdgriffith$elm_ui$Element$spacing(10)
						...
						[
							...
							A2(
									...
									return A2(
										...
										_List_fromArray(
											[
												...
												$mdgriffith$elm_ui$Element$el,
												_List_Nil,
												A3($author$project$Morphir$Visual$ViewValue$viewValue, ctx, $elm$core$Dict$empty, valDef.body)),
												A2(
												$mdgriffith$elm_ui$Element$column,
												_List_fromArray(
													[
														...
														$mdgriffith$elm_ui$Element$padding(10),
														$mdgriffith$elm_ui$Element$Events$onClick(
														A2(ctx.onReferenceClicked, fqName, true))
													]),
												_List_fromArray(
													[
								...
								$elm$core$List$reverse(
									$elm$core$Dict$toList(expandedFunctions))))
						]))
				]));
	});
	...
	switch (model.$) {
		case 'IRLoaded':
			return A2(
				...
				_List_fromArray(
					[
						$elm$html$Html$text('IR Loaded Successfully')
					]));
		case 'Failed':
			var string = model.a;
			return A2(
		...
		default:
			...
			return A2(
				...
				_List_Nil,
				A5($author$project$Morphir$Visual$ViewValue$viewDefinition, visualizationState.distribution, visualizationState.functionDefinition, validArgValues, $author$project$Morphir$Web$Insight$ExpandReference, visualizationState.expandedFunctions));
	}
};
